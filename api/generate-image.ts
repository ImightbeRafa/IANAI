...
function grokUserFacingError(
  ...
  // Never ask the user to shorten a guion — clamp already ran (incl. prompt_too_long).
  return es
...
- LOGO: si hay una imagen de logo adjunta, estamparla tal cual — no redibujar wordmark ni lockup con IA.\n`
    ...
    if (selectedModel === 'grok-imagine') {
      ...
      const preferTailForGrok = isProductMode || isLogoMode || isShellMetaImagePrompt(userPrompt)
        ? ''
        : grokUserCopy
