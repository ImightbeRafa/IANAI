...
import { rehostBrandLogo } from './lib/brand-kit.js'
...
function extractStructuredData(html: string): string {
  ...
  for (const match of html.matchAll(/<script\b[^>]*type\s*=\s*["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    ...
    try {
      ...
      // Some storefronts emit almost-valid JSON-LD. It is still useful evidence for the model.
      blocks.push(raw)
    }
  }
  return blocks.join('\n').slice(0, 8_000)
...
async function synthesizeSite(
  ...
  pages: PageSignals[],
  cssText: string
): Promise<{ facts: Record<string, unknown>; evidence: Record<string, SiteFieldEvidence>; usage: { input: number; output: number; thinking: number } }> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    normalizedUrl = assertPublicHttpUrl(url.trim()).href
  } catch {
  ...
  try {
    ...
    if (extractedLogo) {
      try {
        const hosted = await rehostBrandLogo(user.id, extractedLogo)
    ...
    await logApiUsage({
      ...
      model: MODEL,
      inputTokens: synthesized.usage.input,
      outputTokens: synthesized.usage.output,
      thinkingTokens: synthesized.usage.thinking,
      success: true,
      metadata: { action: 'analyze_site', host: new URL(normalizedUrl).hostname, pages: crawled.pages.length },
    })
    ...
    await logApiUsage({
      ...
      metadata: { action: 'analyze_site', host: new URL(normalizedUrl).hostname },
    })
