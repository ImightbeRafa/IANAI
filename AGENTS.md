...
- **Supabase project:** production + Vercel Preview both target **AIIAN** `lstzfxsdmggkoaxfawny` (policy since 2026-08-22). Do not point new Preview work at isolated IANAI-preview `adrwkzibhfdpwuycnzaa`. See `docs/operations/chat-shell-environments.md`. Preview must keep Deployment Protection on; never apply Preview-only RLS/seed dumps onto AIIAN; never use Preview as a payment webhook target.
