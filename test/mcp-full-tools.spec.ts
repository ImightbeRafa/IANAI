...
describe('mcp 0.8 remaining tools', () => {
  ...
  it('lists enabled tools including new ones, hides disabled and admin from non-admins', async () => {
    ...
    expect(schemaByName.execute_bulk_posts.properties).toHaveProperty('productImageId')
    expect(schemaByName.execute_bulk_posts.properties).toHaveProperty('referenceImageIds')
    expect(schemaByName.execute_bulk_posts.properties).toHaveProperty('referenceMode')
    expect(schemaByName.execute_bulk_posts.properties).toHaveProperty('aspectRatioFallback')
    expect(schemaByName.execute_campaign_pack.properties).toHaveProperty('referenceImageIds')
